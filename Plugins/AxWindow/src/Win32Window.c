...
static AxMessageBoxResult CreateMessageBox(const AxWindow *Window, const char *Title, const char *Message, enum AxMessageBoxFlags Type)
{
    ...
    result.Success = false;
    result.Error = AX_WINDOW_ERROR_NONE;
    result.Response = AX_MESSAGE_BOX_RESPONSE_CANCEL;

    uint32_t Flags = 0;
    if (Type & AX_MESSAGE_BOX_TYPE_ABORTRETRYIGNORE) { Flags |= MB_ABORTRETRYIGNORE; }
    if (Type & AX_MESSAGE_BOX_TYPE_CANCELTRYCONTINUE) { Flags |= MB_CANCELTRYCONTINUE; }
    if (Type & AX_MESSAGE_BOX_TYPE_HELP) { Flags |= MB_HELP; }
    if (Type & AX_MESSAGE_BOX_TYPE_OK) { Flags |= MB_OK; }
    if (Type & AX_MESSAGE_BOX_TYPE_OKCANCEL) { Flags |= MB_OKCANCEL; }
    if (Type & AX_MESSAGE_BOX_TYPE_RETRYCANCEL) { Flags |= MB_RETRYCANCEL; }
    if (Type & AX_MESSAGE_BOX_TYPE_YESNO) { Flags |= MB_YESNO; }
    if (Type & AX_MESSAGE_BOX_TYPE_YESNOCANCEL) { Flags |= MB_YESNOCANCEL; }
    if (Type & AX_MESSAGE_BOX_ICON_EXCLAMATION) { Flags |= MB_ICONEXCLAMATION; }
    if (Type & AX_MESSAGE_BOX_ICON_WARNING) { Flags |= MB_ICONWARNING; }
    if (Type & AX_MESSAGE_BOX_ICON_INFORMATION) { Flags |= MB_ICONINFORMATION; }
    if (Type & AX_MESSAGE_BOX_ICON_QUESTION) { Flags |= MB_ICONQUESTION; }
    if (Type & AX_MESSAGE_BOX_ICON_STOP) { Flags |= MB_ICONSTOP; }
    if (Type & AX_MESSAGE_BOX_ICON_ERROR) { Flags |= MB_ICONERROR; }
    if (Type & AX_MESSAGE_BOX_DEFBUTTON1) { Flags |= MB_DEFBUTTON1; }
    if (Type & AX_MESSAGE_BOX_DEFBUTTON2) { Flags |= MB_DEFBUTTON2; }
    if (Type & AX_MESSAGE_BOX_DEFBUTTON3) { Flags |= MB_DEFBUTTON3; }
    if (Type & AX_MESSAGE_BOX_DEFBUTTON4) { Flags |= MB_DEFBUTTON4; }
    if (Type & AX_MESSAGE_BOX_APPLMODAL) { Flags |= MB_APPLMODAL; }
    if (Type & AX_MESSAGE_BOX_SYSTEMMODAL) { Flags |= MB_SYSTEMMODAL; }
    if (Type & AX_MESSAGE_BOX_TASKMODAL) { Flags |= MB_TASKMODAL; }
